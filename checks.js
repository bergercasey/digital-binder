...
(function(){
  ...
  function findLogHeading(){
    ...
    for (var i=0;i<heads.length;i++){
      ...
      if (t === 'log') return heads[i];
    }
    // fallback: previous sibling of notes list that contains 'log'
    var cont = $('notes-list');
    if (cont){
      var n = cont.previousElementSibling, steps = 0;
      while(n && steps<4){
        if (((n.textContent||'').trim().toLowerCase()).indexOf('log') !== -1) return n;
  ...
  function ensureRowCheckboxes(container){
    var dates = qsa('#notes-list .note-date, .note-item .note-date');
    if (!dates.length && container) dates = qsa('.note-date', container);
    for (var i=0;i<dates.length;i++){
      var d = dates[i];
      if (d.querySelector('input.pe_row_chk')) continue;
      var c = document.createElement('input'); c.type='checkbox'; c.className='pe_row_chk';
      d.appendChild(c);
    }
  }
  ...
  function boot(){
    ...
    try{
      var pb = $('print-job');
      if (pb) pb.textContent = 'Email/Print';
      else {
        qsa('button, a[role="button"]').forEach(function(n){
          var t = (n.textContent||'').trim().toLowerCase();
          if (t === 'print selected') n.textContent = 'Email/Print';
        });
      }
    }catch(e){}
