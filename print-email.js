(function(){
  ...
  cssOnce('pe_css', `
    .pe-select { display:inline-flex; align-items:center; gap:6px; margin-right:8px; }
    ...
    #pe_modal{width:min(92vw,780px);max-height:82vh;overflow:auto;background:#fff;border:1px solid #d0d7de;border-radius:12px;box-shadow:0 10px 30px rgba(0,0,0,.25);padding:14px;font-family:system-ui,-apple-system,Segoe UI,Roboto,sans-serif;color:#111}
    ...
    .pe-list{border:1px solid #d0d7de;border-radius:8px;padding:10px;max-height:38vh;overflow:auto}
  ...
  function ensureSelectAll(){
    ...
    const h = Array.from(document.querySelectorAll('h2,h3,h4')).find(x => (x.textContent||'').trim() === 'Log');
    if (!h || $('#pe_select_all')) return;
    const label = document.createElement('label'); label.className='pe-select'; label.id='pe_select_all';
    const cb = document.createElement('input'); cb.type='checkbox'; cb.id='pe_cb_all';
    label.appendChild(cb); label.appendChild(document.createTextNode('Select all'));
  ...
  function injectRowCheckboxes(){
    const list = $(SEL.list); if (!list) return;
    $all(SEL.row, list).forEach(row => {
      if (row.querySelector('.pe_cb_row')) return;
      const dateEl = row.querySelector(SEL.date) || row.firstChild;
      const lab = document.createElement('label'); lab.className='pe-select';
      const cb = document.createElement('input'); cb.type='checkbox'; cb.className='pe_cb_row';
      lab.appendChild(cb);
      if (dateEl && dateEl.parentNode) dateEl.parentNode.insertBefore(lab, dateEl);
      else row.insertBefore(lab, row.firstChild);
    });
  }

  function getSelectedHTML(){
    const list = $(SEL.list); if (!list) return [];
    const rows = $all(SEL.row, list);
    const chosen = [];
    rows.forEach(r => {
      ...
      if (cb && cb.checked){
        ...
        // strip injected UI
        $all('.pe-select,.pe_chip_btn', clone).forEach(n=>n.remove());
  ...
  function ensureModal(){
    ...
    modal.innerHTML = `
      ...
      <div style="display:flex;gap:14px;flex-wrap:wrap">
        <div class="pe-col">
          <div class="pe-muted" style="margin-bottom:6px">Recipients</div>
        ...
        <div class="pe-col">
          <div class="pe-muted" style="margin-bottom:6px">Preview</div>
  ...
  function parseContact(s){
    s = (s||'').trim();
    if (!s) return null;
    // "Name <email@x.com>" or just email
    const m = /^(.*)<([^>]+)>$/.exec(s);
  ...
  function addAddressFromInput(){
    const box = $('#pe_addr_input'); const val = box.value.trim();
    const c = parseContact(val); if (!c) return;
    const list = $('#pe_addr_list');
    const chip = contactChip(c);
  ...
  function openModal(){
    ...
    if (!chosen.length){ alert('Check Select all or individual log boxes first.'); return; }
    ensureModal();
    $('#pe_preview').innerHTML = chosen.join('');
  ...
  async function doEmail(){
    ...
    const intro = '<p><strong>'+(who?.user||'A user')+'</strong> added info to <strong>'+jobTitle+'</strong>.</p>';
    ...
    alert('Sent!');
    $('#pe_overlay').style.display='none';
  }

  function retitlePrintButton(){
    const btn = document.querySelector(SEL.printBtn);
    if (!btn) return;
    if (!btn.dataset._peBound){
      btn.textContent = 'Print/Email';
      btn.addEventListener('click', (e)=>{ e.preventDefault(); openModal(); });
      btn.dataset._peBound = '1';
    }
  }
  ...
  // initial + observe list re-renders
