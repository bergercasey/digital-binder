// save-manager.js — Save HUD + cloud-confirmed saving (build3-final)
(function(){
  ...
  function hud(state,text){
    ...
    el.hidden=false;
    if(state==='ok') setTimeout(()=>{ el.hidden=true; },1200);
  }
  ...
  const SaveManager=(function(){
    ...
    async function tick(){
      ...
      const payload = { ...snapshot, serverVersion };

      inFlight = true;
      hud('saving','Saving to cloud…');
      try{
        ...
        pending = snapshot;           // keep latest snapshot queued
        setTimeout(tick, 1500);       // retry
      }finally{
        inFlight=false;
        if(pending) setTimeout(tick, 75);
      }
    }
    return { init, queueSave, get serverVersion(){ return serverVersion; } };
  })();

  function detectSerializer(){
    if(typeof window.serializeAppState === 'function') return window.serializeAppState;
    console.warn('[save-manager] serializeAppState() not found — define window.serializeAppState to enable saving.');
    return null;
  }
  ...
  function boot(){
    ...
    const debounced = (function(fn,ms){ let t; return ()=>{ clearTimeout(t); t=setTimeout(fn,ms); }; })(()=>{
      try{
        ...
        hud('error','serializeAppState() failed');
        console.error(err);
      }
    }, 250);

    // Any input/change triggers a debounced save
