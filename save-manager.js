// build2: Save HUD + smart client save manager
(function(){
    ...
    async function tick(){
      ...
      try{
        ...
        if(r.status===200 && body && body.ok){
          ...
          hud('error','Save rejected (guarded payload)');
  ...
  function detectSerializer(){
    ...
    if(window.app && typeof window.app.serialize === 'function') return ()=>window.app.serialize();
    if(window.app && typeof window.app.export === 'function') return ()=>window.app.export();
    console.warn('[build2] serializeAppState() not found. Define window.serializeAppState to enable saving.');
    return null;
  }
  ...
  function boot(){
    ...
    window.queueSaveNow = ()=>{
      try{ const snap = serializer(); SaveManager.queueSave(snap); }catch(e){ console.error(e); }
    };
