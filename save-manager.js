// save-manager.js — Save HUD + cloud-confirmed saving (build4)
(function(){
  ...
  #saveHud.ok{background:#124c20}#saveHud.saving{background:#141826}#saveHud.offline{background:#593100}#saveHud.stale{background:#5a102a}#saveHud.error{background:#5a1010}`;
  function injectHUD(){
    if(document.getElementById('saveHud')) return;
    const style=document.createElement('style');style.textContent=CSS;document.head.appendChild(style);
    const hud=document.createElement('div');hud.id='saveHud';hud.hidden=true;
    const span=document.createElement('span');span.id='saveHudText';span.textContent='Saving…';
  ...
  function hud(state,text){const el=$hud();if(!el)return;el.className=state;if($txt())$txt().textContent=text||'';el.hidden=false;if(state==='ok')setTimeout(()=>{el.hidden=true;},1200);}
  ...
  const SaveManager=(function(){
    ...
    async function init(){
      try{
        ...
        if(r.ok){ const data=await r.json(); serverVersion=(data&&data.serverVersion)||0; window._serverVersion=serverVersion; }
      }catch(_){}
    }
    ...
    async function tick(){
      ...
      try{
        ...
        if(r.status===200 && body && body.ok){
          ...
          hud('error','Save rejected'); console.warn('Save 400',raw);
        }else{
          hud('error',`Save error ${r.status}`); console.warn('Save error',r.status,raw);
        }
      }catch(e){
        hud('offline','Offline — will retry…'); pending=snapshot; setTimeout(tick,1500);
      }finally{
        inFlight=false; if(pending) setTimeout(tick,75);
      }
    }
    return { init, queueSave, get serverVersion(){ return window._serverVersion ?? serverVersion; } };
  })();

  function detectSerializer(){
    if(typeof window.serializeAppState==='function') return window.serializeAppState;
    console.warn('[save-manager] serializeAppState() missing'); return null;
  }
  function boot(){
    injectHUD(); SaveManager.init();
    const serializer=detectSerializer();
    ...
    const debounced=(function(fn,ms){let t;return()=>{clearTimeout(t);t=setTimeout(fn,ms);};})(()=>{
      try{ const snap=serializer(); if(!snap||typeof snap!=='object'){ hud('error','serializeAppState() returned nothing'); return; }
    ...
    window.queueSaveNow=()=>{ try{ const s=serializer(); SaveManager.queueSave(s);}catch(e){ console.error(e);} };
  }
