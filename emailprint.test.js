console.log('emailprint.test.js — LIVE 20250924-022317');
...
(function(){
  ...
  console.log('selectedNotes (modal) ->', out.length);
  return out;
}:\d{2}.*$/,'').trim();
...
function openModal(){
  ...
  // Close overlay so nothing stacks/darkens
  try { if (ov && ov.parentNode) ov.parentNode.removeChild(ov); } catch(_){}

  // Read ONLY checked notes from the MODAL
  var notes = selectedNotes();
  if (!notes.length){ alert('Select at least one log entry.'); return; }
  ...
  // Build HTML, preserving bullets when we have innerHTML; otherwise convert newlines
  var info = jobInfo();
  function esc(s){ return String(s||'').replace(/[&<>]/g,function(c){return ({'&':'&amp;','<':'&lt;','>':'&gt;'}[c]);}); }
  ...
  function intercept(){
  function handler(e){
    var target = e.target && e.target.closest ? e.target.closest('#print-job') : null;
    if (!target) return;
    e.preventDefault();
