/* emailprint STRICT build 20250924-111539 */
console.log('emailprint.strict — LIVE 20250924-111539');
...
(function(){
  ...
  console.log('selectedNotes (overlay) ->', out.length);
  return out;
}:\d{2}.*$/,'').trim();
...
function openModal(){ console.log('EP modal opened (STRICT)');
  var info = jobInfo();
  ...
  // Collect selection from the MODAL first
  var notes = selectedNotes();
  if (!notes.length){ alert('Select at least one log entry.'); return; }

  // Close overlay so the screen doesn't darken/stack and to prevent intercepts
  ...
  // Build HTML, preserving bullets when innerHTML includes lists; otherwise convert line breaks
  var info = jobInfo();
  function esc(s){ return String(s||'').replace(/[&<>]/g,function(c){return ({'&':'&amp;','<':'&lt;','>':'&gt;'}[c]);}); }
  ...
  var items = notes.map(function(n){
    var content = (n.html && n.html.indexOf('<li') !== -1) ? n.html : ((n.html && n.html.trim()) ? n.html : txtToHtml(n.text||''));
  ...
  // Show and immediately trigger OS Print
