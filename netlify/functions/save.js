...
export async function handler(event){
  ...
  try{
    ...
    if(!allowEmpty){
      ...
      if(keys.length<2){
        return { statusCode:400, headers:{'Content-Type':'application/json','Cache-Control':'no-store'}, body: JSON.stringify({ok:false, reason:'suspicious-empty'}) };
      }
    }
    ...
    if(incVer<currVer){
      return { statusCode:409, headers:{'Content-Type':'application/json','Cache-Control':'no-store'}, body: JSON.stringify({ok:false, reason:'stale', serverVersion:currVer}) };
    }
    ...
    return { statusCode:200, headers:{'Content-Type':'application/json'}, body: JSON.stringify({ok:true, serverVersion: next.serverVersion}) };
