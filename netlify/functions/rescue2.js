...
export async function handler() {
  ...
  <title>Rescue & Restore (override)</title>
  <style>
  ...
  <div>
    ...
    <p>Select your JSON and click restore. This uses <code>save-override</code> which bumps version to server+1 and always writes.</p>
    <div class="row">
      <input type="file" id="file" accept="application/json">
      <button id="restore">Restore to server (override)</button>
    </div>
  ...
  $('#restore').addEventListener('click', async ()=>{
    ...
    try{
      ...
      if (json['binder-data']) { try{ json = JSON.parse(json['binder-data']); }catch{} }
      else if (json['digital-binder']) { try{ json = JSON.parse(json['digital-binder']); }catch{} }

      // We don't set serverVersion here; save-override will bump it above current.
      const r = await fetch('/.netlify/functions/save-override', {
        method:'POST',
