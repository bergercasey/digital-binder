...
export async function handler(event){
  ...
  if(!needAuth()) return { statusCode:200, headers:{'Content-Type':'application/json'}, body: JSON.stringify({ok:true,user:'anon'}) };
  ...
  const parts = [`binder_auth=${token}`, 'Path=/', 'HttpOnly', 'SameSite=Lax', 'Secure']; if(remember) parts.push('Max-Age=1209600');
