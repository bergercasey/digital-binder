...
(function(){
  ...
  let state = {
    ...
    ui: { selectedContractorId: null, selectedJobId: null, view: "main", showArchived: false, editing: false }
  };
...
function renderAll() {
  ...
  function wire() {
    statusEl = $("status");
    // Toggle Edit/Done for job settings
    const editBtn = $("edit-job");
    if (editBtn) {
      const updateLabel = () => { editBtn.textContent = state.ui.editing ? "Done" : "Edit Job"; };
      updateLabel();
      editBtn.addEventListener("click", () => {
        const j = currentJob(); if (!j) return;
        // If turning OFF editing, consider setup complete to start logging subsequent changes
