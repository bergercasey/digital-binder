...
btnEmail.addEventListener('click', async function(){
      ...
      notes.forEach(function(n){
        textParts.push(n.date);
        textParts.push(n.text);
        textParts.push('');
      });
      var textBody = textParts.join('\n');

      // HTML body mirrors the print layout (inline styles for email clients)
      ...
      var html = '<!doctype html><html><head>' +
        ...
        '</head><body style="font:16px/1.4 -apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Ubuntu,sans-serif;color:#111;padding:16px">';

      if (info.name) html += '<h1 style="font-size:20px;font-weight:600;margin:0">'+ esc(info.name) +'</h1>';
      if (info.address) html += '<div style="margin:2px 0 8px 0">'+ esc(info.address) +'</div>';
      if (info.po || info.stage){
        html += '<div style="margin:0 0 16px 0">';
        if (info.po) html += '<div style="margin:2px 0">PO: '+ esc(info.po) +'</div>';
        if (info.stage) html += '<div style="margin:2px 0">Stage: '+ esc(info.stage) +'</div>';
        html += '</div>';
      }

      notes.forEach(function(n){
        html += '<div style="border:1px solid #e5e7eb;border-radius:10px;padding:12px;margin:14px 0">' +
...
btnPrint.addEventListener('click', function(){
      ...
      html+='<script>(function(){var p=false;function c(){try{window.close()}catch(_){}}window.onafterprint=function(){p=true;c()};window.addEventListener("focus",function(){if(p)c()});setTimeout(function(){window.print()},50)})();<\/script></body></html>';
