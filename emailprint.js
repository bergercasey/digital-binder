...
(function(){
  ...
  function openModal(){
    ...
    var addInput=document.createElement('input'); addInput.placeholder='Name <email@domain> or email@domain'; addInput.style.flex='1'; addInput.type='text';
    ...
    addBtn.addEventListener('click', function(){
      ...
      if(m){ contacts.push({name:m[1]||m[2], email:m[2]}); }
      else if(/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(v)){ contacts.push({name:v, email:v}); }
      else { alert('Use: Name <email@domain> or plain email'); return; }
  ...
  // ---- Intercept original print button, don't remove it ----
