...
btnEmail.addEventListener('click', async function(){
  var picks = qsa('.ep_rec', listWrap).filter(x => x.checked).map(x => x.value);
  ...
  textParts.push(''); notes.forEach(n => { textParts.push(n.date); textParts.push(n.text); textParts.push(''); });
  var textBody = textParts.join('\n');

  var html = window.__lastPrintHTML && window.__lastPrintHTML.length ? window.__lastPrintHTML : __ep_buildHTML(info, notes);
...
// === EPv22 shared builder (print & email are identical) ===
function __ep_buildHTML(info, notes){
  ...
  function toBullets(s){
    ...
    if (parts.length > 1){
      return '<ul style="margin:0;padding-left:22px;list-style:disc">' + parts.map(t => '<li>'+ esc(t) +'</li>').join('') + '</ul>';
    }
