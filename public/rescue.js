...
$('#restore').addEventListener('click', async () => {
  ...
  try {
    ...
    // If user exported raw localStorage keys, try common key names
    if (json['binder-data']) {
      try { json = JSON.parse(json['binder-data']); } catch (_) {}
    } else if (json['digital-binder']) {
      try { json = JSON.parse(json['digital-binder']); } catch (_) {}
    }
    // Ensure huge serverVersion so it wins; server will increment
