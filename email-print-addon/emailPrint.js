/* Email/Print Add-on (Build 1759417591)
 * Promise: This script DOES NOT change, remove, disable, or hook into your Delete button's behavior.
 * It ONLY finds that button in the DOM and inserts a new Email/Print button immediately after it.
 */
(function(){"use strict";
  ...
  function findDeleteButton(){
    ...
    // Fallback by text content (non-destructive)
  ...
  function getField(list){
    const sels = list.split(',').map(s=>s.trim());
  ...
  function insertButtonNextToDelete(){
    ...
    // Create a NEW button; do NOT alter the delete button at all.
    const btn = document.createElement('button');
    btn.id = 'emailPrint';
    btn.textContent = 'Email/Print';
    btn.style.marginLeft = '8px';
    // Try to borrow styling classnames without hooking any handlers on delete
